...
public class OrderedList_inArraySlots implements OrderedList {
  ...
  /**
   * construct order from an unordered ArrayList. Elements may not be null.
   */
  public OrderedList_inArraySlots(ArrayList<Integer> unordered) {
    list_iAS = unordered;
    for (int i = 0; i < list_iAS.size(); i++) {
      int champIndex = champIndex(i, list_iAS.size());
      swap(i, champIndex);
    }
  }

  /**
   * Takes a range over list_iAS and finds the smallest element in the range [startIndex, endIndex)
   * 
   * @return the index of smallest element in the range
   */
  private int champIndex(int startIndex, int endIndex) {
    Integer smallest = list_iAS.get(startIndex);
    int smallestIndex = 0;
    int i = startIndex;
    for(; i < list_iAS.size(); i++) {
      if (list_iAS.get(i).compareTo(smallest) < 1) {
        smallest = list_iAS.get(i);
        smallestIndex = i;
      }
    }
  ...
  private void swap(int index, int secondIndex) {
    Integer temp = list_iAS.get(index);
    list_iAS.set(index, list_iAS.get(secondIndex));
    list_iAS.set(secondIndex, temp);
  }
